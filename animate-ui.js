...
(function () {
  ...
  function buildTracks() {
    ...
    rows.forEach(function (r, i) {
      if (r.kind === 'clip') {
        ...
        // Each segment of the reel, numbered, so a six-cut breakdown reads at
        // a glance instead of living in the export.
        var segs = '';
  ...
  // Setting one end past the other takes the other with it rather than being
  // clamped back. Cutting a second segment later in the game means pressing I
  // beyond the previous out, and clamping made that silently pick the wrong
  ...
  function markOut() {
    outMs = clamp(tNow, 200, T);
    if (inMs > outMs - 200) inMs = Math.max(0, outMs - 200);
    syncTimeline(true); render();
  }
  ...
  drawRinkBg = function (p) {
    ...
    ctx.save();
    // The break card: black with the crest, so a jump to another part of the
    // game reads as a new clip rather than a glitch.
    if (breakUntil) {
      ...
      ctx.strokeStyle = '#343A43'; ctx.lineWidth = 1;
  ...
  syncScrub = function () {
    ...
    // then the reel: while a card is up the clip is paused and its clock must
    // not be read back over tNow
    var onCard = advanceReel();
    if (onCard) { _syncScrub(); return; }
    if (vid && vid.duration) {
  ...
  // ---------------------------------------------------------
  // The reel: several segments cut from one long game clip, played and
  // exported back to back with a crest card between them. Two plays five
  // minutes apart look like a jump cut otherwise — the card says "new clip".
  // ---------------------------------------------------------
  var segments = [];          // [{in, out}] in the order they play
  var breakMs = 3000;
  ...
  // Export size, best to smallest. A full-quality 1080p reel ran past a
  // gigabyte, over Slack's limit and slow on a phone. Phone is the default.
  // Chrome's encoder overshoots the rate it is given, so these targets are
  // set from measured files, not from the arithmetic.
  var SIZES = {
    full:  { capW: 0,    rate: 0,       say: 'Full: the clip\'s own resolution, biggest file' },
    sharp: { capW: 1280, rate: 3500000, say: 'Sharp: 720p, high quality' },
    phone: { capW: 1280, rate: 1500000, say: 'Phone: 720p, made for sending to players' },
    quick: { capW: 960,  rate: 700000,  say: 'Quick: 540p, the smallest and fastest to send' }
  };
  var exportSize = 'phone';
  try {
    var es = localStorage.getItem('kd.exportSize');
    if (SIZES[es]) exportSize = es;
  } catch (e) { }
  function paintSize() {
  ...
  [].slice.call($('kdSize').querySelectorAll('button')).forEach(function (b) {
    b.addEventListener('click', function () {
      ...
      toast('Export size: ' + SIZES[exportSize].say);
    });
  });
  paintSize();
  ...
  function exportClip() {
    ...
    var sz = SIZES[exportSize] || SIZES.phone;
    var small = !!sz.rate;
    var native = useNativeView(sz.capW);
    ...
    var rate = small ? sz.rate : Math.max(8000000, Math.round(px * 60 * 0.12));
    try {
    ...
    var begin = function () {
      ...
      var watch = setInterval(function () {
        // On a reel, advanceReel() decides when it is over — it walks the
        // segments and holds the cards, and stops playback at the last one.
        if (onReel()) {
          if (!playing) {
            clearInterval(watch);
            render();
            setTimeout(function () { if (rec.state !== 'inactive') rec.stop(); }, 160);
          }
          return;
        }
